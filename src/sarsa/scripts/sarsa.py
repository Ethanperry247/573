...
def sarsa_learning(episodes, read_table_from_file, base_d, file_read_number, file_write_number):
    global left, front, right, diagonal
    global pose
    global last_scan
    Qt2 = None
    if (read_table_from_file):
        Qt2 = json.load(open("tables/qt" + str(file_read_number) + ".json", "r"))
    else:
    ...
    for episode in range(episodes):
        ...
        # Reset the simulation and get the new initial state and action.
        previous_state_index = initial_state()
        previous_action_index = greedy_epsilon(Qt2, previous_state_index, episode + base_d, EPSILON_0, D_GREEDY, TOTAL_ACTIONS)
        ...
        while not terminal_condition:
            ...
            take_action(previous_action_index)
            ...
            state_index = int(diagonal.value * STATE_DIMENSIONS[1] * STATE_DIMENSIONS[2] * STATE_DIMENSIONS[3] + left.value * STATE_DIMENSIONS[2] * STATE_DIMENSIONS[3] + front.value * STATE_DIMENSIONS[3] + right.value)

            # Obtain a new action
            action_index = greedy_epsilon(Qt2, state_index, episode + base_d, EPSILON_0, D_GREEDY, TOTAL_ACTIONS)

            # Update Q table as according to sarsa algorithm.
            Qt2[previous_state_index][previous_action_index] = Qt2[previous_state_index][previous_action_index] + ALPHA * (reward + GAMMA * Qt2[state_index][action_index] - Qt2[previous_state_index][previous_action_index])
            ...
            # After 1000 to 2000 iterations without crashing into the wall, commense the next episode.
            iteration += 1
            if (iteration > 1500):
        ...
        print("Average Reward: " + str(float(total_reward) / float(iteration)))

        # Save to the JSON file with each episode
        json.dump(Qt2, open("tables/qt" + str(file_write_number) + ".json", "w"))
...
def reinforcement():
    ...
    table = sarsa_learning(10000, True, 100000, 102, 103)
