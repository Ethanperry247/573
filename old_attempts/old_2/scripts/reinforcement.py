...
FRONT_SCAN_INDICES = [354, 355, 356, 357, 358, 359, 0, 1, 2, 3, 4, 5]
RIGHT_SCAN_INDICES = [264, 265, 266, 267, 268, 269, 270, 271, 272, 273, 274, 275]
LEFT_SCAN_INDICES = [84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95]
...
def callback(scan):
    ...
    determine_state(create_average_measurement(scan, LEFT_SCAN_INDICES), 0, 0.67, 0.7)
    determine_state(create_average_measurement(scan, RIGHT_SCAN_INDICES), 2, 0.67, 0.7)
    determine_state(create_average_measurement(scan, FRONT_SCAN_INDICES), 1, 0.67, 0.7)
...
def determine_state(distance, region, close_boundary, far_boundary):
    ...
    # print("Region:  " + str(region) + "    Distance:   "+ str(distance))
...
def q_learning(episodes, read_table_from_file, base_d):
    ...
    if (read_table_from_file):
        Qt2 = json.load(open("qt14.json", "r"))
    else:
    ...
    for episode in range(episodes):
        ...
        json.dump(Qt2, open("qt15.json", "w"))
