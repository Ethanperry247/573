...
DIAGONAL_FRONT_LEFT = [44, 45, 46]
DIAGONAL_FRONT_RIGHT = [314, 315, 316]
DIAGONAL_BACK_RIGHT = [224, 225, 226]
DIAGONAL_BACK_LEFT = [134, 135, 136]
...
def rotate_sensors_left():
    ...
    right = copy.deepcopy(RIGHT_SCAN_INDICES)
    RIGHT_SCAN_INDICES = copy.deepcopy(FRONT_SCAN_INDICES)
    FRONT_SCAN_INDICES = copy.deepcopy(LEFT_SCAN_INDICES)
    LEFT_SCAN_INDICES = copy.deepcopy(BACK_SCAN_INDICES)
    BACK_SCAN_INDICES = right

def rotate_sensors_right():
    change_virtual_orientation(False)
    global FRONT_SCAN_INDICES, RIGHT_SCAN_INDICES, LEFT_SCAN_INDICES, BACK_SCAN_INDICES
    back = copy.deepcopy(BACK_SCAN_INDICES)
    BACK_SCAN_INDICES = copy.deepcopy(LEFT_SCAN_INDICES) 
    LEFT_SCAN_INDICES = copy.deepcopy(FRONT_SCAN_INDICES)
    FRONT_SCAN_INDICES = copy.deepcopy(RIGHT_SCAN_INDICES)
...
def callback(scan):
    ...
    print("Front Indices: " + str(FRONT_SCAN_INDICES))
    print("Back Indices: " + str(BACK_SCAN_INDICES))
...
def q_learning(episodes, read_table_from_file, base_d, file_read_number, file_write_number):
    ...
    for episode in range(episodes):
        ...
        while not terminal_condition:
            ...
            print("Left:    " + str(left) + "   Front:   " + str(front)+ "  Right:   " + str(right) + " Diagonal:   " + str(diagonal) + "    Next State: " + str(Qt2[next_state_index]) + "    State Index: " + str(next_state_index))
